# Microtastic — Agent & Contributor Guide
...
- **No agentic session artifacts** — checkbox-driven plan files, tool-specific session directories, and scaffold state generated by any agentic tool are session-local only; never commit them. Design docs belong in `docs/vX.Y.Z/<feature>-plan.md`.
